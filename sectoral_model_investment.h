...
*/
	...
	CYCLE_SAFE(cur1, "CAPITALS")
	{
		...
		if((double)t>=v[5]&&v[1]>1)													//if the current time step is higher than the date of birth of the capital good plus the depreciation period, this capital good must be depreciated
			{
...
*/
	...
	if(v[7]==1&&v[3]>0)
		...
		CYCLE(cur, "CAPITALS")																																					// CYCLE trought capital goods
   			{
     		v[9]=VS(cur, "capital_good_to_replace");
     		if(v[9]==1)
     			{
     			WRITES(cur, "capital_good_productivity_initial", v[1]);				//writes the new capital productivity as the frontier productivity when it was ordered
				WRITES(cur, "capital_good_date_birth", t);					//writes the new capital date of birth as the current time period
...
*/
	...
	if(v[9]==1)														//if it is investment period for the firm
		...
		v[4]=((v[0]*(1+v[2]))/v[3]) - v[1];							//desired productive capacity will be the amount needed based on expected sales minus existing plus what will depreciate in the next investment period
 ...
 v[13]=V("payback_period");
  ...
  SORT("CAPITALS","Capital_Good_Productivity","UP");																								// sort capital goods from the lowest to highest 
   CYCLE(cur, "CAPITALS")																																					// CYCLE trought capital goods
   {
     ...
     if((double)t!=v[19] && (double)t < v[19])																					// if the capitalgood was not created in the current period nor will depreciate in the next investment period
     {
     ...
     v[23]=v[11]*(1+v[2])/(v[10]*((1/(v[17]))-(1/(v[8]))));																								// calculates the payback           
         if(v[8]>v[17] && v[23]<=v[13]*v[9])																														// if the cost of replacement is lower than current available funds and the paybakc calculus  is lower than the payback parameter
...
//Financial Constraints happen here, see fun.finance.h
	...
	if(v[3]==1)														// if it is investment period for the firm
	{
		...
		CYCLE(cur, "CAPITALS")										// CYCLE trought capital goods
		{
			...
			if((double)t!=v[19] && (double)t < v[19]-v[1])		// if the capitalgood was not created in the current period nor will depreciate in the next investment period
				...
				if(v[8]>v[17] && v[20]<=v[15] && v[23]<=v[13]*v[9])		// if the cost of replacement is lower than current available funds and the paybakc calculus  is lower than the payback parameter
					{
...
EQUATION("Firm_Effective_Replacement_Investment_Expenses")
/*
...
*/
v[0]=V("Firm_Effective_Expansion_Investment_Expenses");
v[1]=V("Firm_Effective_Replacement_Investment_Expenses");
v[2]=v[0]+v[1];
RESULT(v[2])
...
*/
	...
	v[2]=V("Firm_Effective_Replacement_Investment_Expenses");
	v[3]=max(0, (v[1]-v[2]));
RESULT(v[3])
...
*/
	...
	if(v[0]==1)
	{
		...
		v[4]=v[2]+v[3];																		//sum up firm's lagged  desired productive capacity to replace plus productive capacity to expand																		//divides the total amount of demand by the invesmet period. This is because capital goods take a whole investment period to be produced. This distributed the demand for the capital goods firms equally during the investment period. It does not mean that each firm demands 1/6 of capital each period, it's just to distribute the production															
		v[5]=V("capital_output_ratio");															//capital outrput ratio
		v[6]=v[5]*v[4];																			//number of capital goods to be demanded (produced) in each production period inside the investment period
	}
	else
		v[6]=0;
RESULT(v[6])
